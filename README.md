...
Альтернативы если хук не установлен или не захватывает stdout:

```bash
service nginx status 2>&1 | wtf      # пайп — самый надёжный, ловит ровно то, что в окне
wtf --rerun                          # перезапустит последнюю команду и поймает stdout/stderr
