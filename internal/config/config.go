...
// первая — дефолт (быстрая+дешёвая, с большим TPM), дальше — «помощнее».
//
// ВАЖНО: для tool-use агента нужны модели с большим TPM-лимитом
// (контекст с tool_results быстро растёт). Не включаем в список модели
// которые зажимают TPM до 30k на низких tier'ах (например gpt-4.1):
// агенту с ними становится невозможно работать на дефолтных квотах OpenAI.
// Юзер всё ещё может ввести их вручную если у него высокий tier.
var KnownModels = map[Provider][]string{
	...
	ProviderOpenAI: {
		"gpt-4o-mini", // дефолт: дешёвая, быстрая, TPM 200k+
