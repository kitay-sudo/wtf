	...
	return []provider.Tool{
		{
			...
			Description: "Запустить безопасную read-only команду на машине пользователя и получить её stdout/stderr. " +
				...
				"ЖЁСТКИЙ ЛИМИТ: максимум 5 команд за один раунд. Больше — система откажет, " +
				"чтобы не упереться в TPM провайдера.",
			...
			Schema: map[string]any{
				...
				"properties": map[string]any{
					...
					"notes": map[string]any{
						"type": "array",
						"description": "ОБЯЗАТЕЛЬНО передай 1-3 факта о машине/сервисах/проблеме которые стоит запомнить надолго. " +
							"Это критично — без notes ассистент в следующий раз будет диагностировать с нуля. " +
							"Примеры: версия сервиса, нестандартный путь конфига, найденная и решённая проблема, " +
							"специфика настройки на этой машине. Каждая запись — короткая (до 200 символов). " +
							"Не дублируй то что уже есть в системной памяти.",
