...
7. **ОБЯЗАТЕЛЬНО запоминай.** В finish.notes ВСЕГДА передавай 1-3 факта.
   Это критично — без памяти агент в следующий раз диагностирует с нуля.
   Что записывать:
     - найденная проблема и её решение → resolved_issue
     - стабильные факты о машине (порты, пути конфигов, версии) → machine_fact
     - текущее состояние сервиса (active/inactive, домены, юзеры) → service_state
     - привычки юзера (если заметил) → user_preference
   Если совсем нечего запоминать — запиши хотя бы 1 resolved_issue с описанием
   что юзер спрашивал и что ты выяснил. Пустые notes — это баг твоей работы.
